...
class AgenticRAGTests(TestCase):
    ...
    def test_retrieve_context_includes_global(self):
        ...
        with patch('core.services.agent.embed_text', return_value=[[0.1]*384]), \
            ...
            context = agent.retrieve_context("What should I do next week?", user_id=1, intent="personal")
            self.assertIn("global", context)
            self.assertIn("personal", context)
            self.assertEqual(context["global"], ['Industry advice 1', 'Industry advice 2'])
            self.assertEqual(context["personal"], ['My show 1', 'My show 2'])
