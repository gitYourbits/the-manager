        ...
        if create_action_button("New Conversation", "➕", "new_conv"):
            try:
                response = requests.post(
                    "http://localhost:8000/api/auth/chat/conversations/",
        ...
        with st.container():
            ...
            if prompt := st.chat_input("Type your message here...", key="chat_input"):
                try:
                    ...
                    response = requests.post(
                        "http://localhost:8000/api/auth/chat/messages/",
                        headers={"Authorization": f"Bearer {st.session_state.token}"},
                        json={
                    ...
                    if response.status_code == 201:
                        ...
                        conv_response = requests.get(
                            f"http://localhost:8000/api/auth/chat/conversations/{st.session_state.current_conversation['id']}/",
                            headers={"Authorization": f"Bearer {st.session_state.token}"}
                        )
